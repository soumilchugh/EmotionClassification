...
def buildGraph(maximum, learning_rate=None, lossType=None, beta1=None, beta2=None, epsilon=None): # Your implementation here
    #print maximum
    new_W = tf.Variable(tf.truncated_normal(tf.zeros([maximum,10000]).shape, stddev=0.05, dtype=tf.float32, name = "initial_W"))
    ...
    predicted_outputs = tf.nn.relu(predicted_outputs, name ='ReLU')
    new_W1 = tf.Variable(tf.truncated_normal(tf.zeros([10000,10000]).shape, stddev=0.05, dtype=tf.float32, name = "initial_W1"))
    b1 = tf.Variable(0.0,name = "initial_b1")
    y1_ = tf.matmul(predicted_outputs, new_W1)
    predicted_outputs1 = tf.math.add( y1_, b1, name="predicted_outputs1")
    predicted_outputs1 = tf.nn.relu(predicted_outputs1, name ='ReLU1')
    new_W2 = tf.Variable(tf.truncated_normal(tf.zeros([10000,4]).shape, stddev=0.05, dtype=tf.float32, name = "initial_W2"))
    b2 = tf.Variable(0.0,name = "initial_b2")
    y2_ = tf.matmul(predicted_outputs1, new_W2)
    predicted_outputs2 = tf.math.add( y2_, b2, name="predicted_outputs")
    #predicted_outputs2 = tf.nn.relu(predicted_outputs2, name ='ReLU2')
    ...
    cost1 = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=predicted_outputs2, labels=trainlabels))
    reg = tf.multiply(lamb / 2, tf.reduce_sum(tf.square(new_W2)))
    reg1 = tf.multiply(lamb / 2, tf.reduce_sum(tf.square(new_W1)))
    reg2 = tf.multiply(lamb / 2, tf.reduce_sum(tf.square(new_W)))
    totalLoss = cost1 + reg + reg1 + reg2
    optimizer = tf.train.AdamOptimizer(learning_rate = learning_rate, name='Adam').minimize(totalLoss)  # Initialise the adam optimizer
    return new_W, b, traindata, predicted_outputs2, trainlabels, lamb, optimizer, totalLoss
