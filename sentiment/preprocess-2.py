...
def measure_Accuracy(new_W, new_b, testData, testLabels):
    ...
    Y_prediction1 = Y_prediction.T
    for i in range(Y_prediction1.shape[0]):
        maximumpredicition = max(Y_prediction1[i][0], Y_prediction1[i][1], Y_prediction1[i][2], Y_prediction1[i][3])
        maximumLabel = max(testLabels[i][0],testLabels[i][1], testLabels[i][2], testLabels[i][3])
        if maximumpredicition == Y_prediction1[i][0]:
            labelprediction = 0
        elif maximumpredicition == Y_prediction1[i][1]:
            labelprediction = 1
        elif maximumpredicition == Y_prediction1[i][2]:
            labelprediction = 2
        elif maximumpredicition == Y_prediction1[i][3]:
            labelprediction = 3
        ...
        if Y_prediction1[i][0] >= 0.5:
            Y_prediction1[i][0] = 1
        else:
            Y_prediction1[i][0] = 0
        if Y_prediction1[i][1] >= 0.5:
            Y_prediction1[i][1] = 1
        else:
            Y_prediction1[i][1] = 0
        if Y_prediction1[i][2] >= 0.5:
            Y_prediction1[i][2] = 1
        else:
            Y_prediction1[i][2] = 0
        if Y_prediction1[i][3] >= 0.5:
            Y_prediction1[i][3] = 1
        else:
            Y_prediction1[i][3] = 0
        totalcount += 1
    ...
    accuracy = np.mean(abs(Y_prediction1 - testLabels))
    ...
    with open("binned_data-2.csv") as f:
        ...
        for row in reader:
            ...
            if l < len(tokenizer.tokenize(text)):
                ...
                print l
    f.close()
    trainLabels1 = np.array(trainLabels[:30000])
    trainData1 = np.array(trainData[:30000])
    validData = np.array(trainData[60000:70000])
    validLabels = np.array(trainLabels[60000:70000])
    tf.reset_default_graph()
    with tf.Session() as session:
        W, b, traindata, predicted_outputs, trainlabels, lamb1, optimizer, loss = buildGraph(maximum,0.001,"CE")
        init = tf.global_variables_initializer()
        session.run(init)
        start = time.time()
        ...
        #print session.run(tf.math.confusion_matrix(labels = trainLabels ,predictions = predicted_outputs))
